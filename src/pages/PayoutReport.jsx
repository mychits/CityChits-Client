...
const PayOutReport = () => {
  ...
  useEffect(() => {
    ...
    //fetchTodaysPayments();
    //   const fetchAllPaymentsReport = async () => {
    //     try {
    //       setTablePayments([]);
    //       setIsLoading(true);

    //       const response = await api.get("/payment-out/get-all-payment-out-report", {
    //         params: {
    //          // from_date: today,
    //          // to_date: today,
    //           pay_for: paymentType,
    ...
    //         // Merge all payments into one array
    //         const mergedPayments = [
    //           ...data.allPayments,
    //           ...data.commissionPayments,
    //           ...data.salaryPayments,
    //           ...data.pettyCashPayments,
    //          // ...data.auctionWinningPayments,
    //         ];

    //         const formattedData = mergedPayments.map((group, index) => ({
    //   _id: group._id,
    //   id: index + 1,
    //   name: group?.user_id?.full_name || "-",
    //   phone_number: group?.user_id?.phone_number || "-",
    ...
    //   date: formatPayDate(group.pay_date),
    //   transaction_date: formatPayDate(group.createdAt),
    //   disbursed_by: group?.admin_type?.full_name || "Super Admin",
    //   disbursement_type: group.disbursement_type || "-",
    //   note: group.note || "-",
    // }));
    ...
    //       }
    //     } catch (error) {
    //       console.error("Error fetching all payment report:", error);
    //       setFilteredAuction([]);
    //     } finally {
    //       setIsLoading(false);
    //     }
    //   };
    const fetchAllPaymentsReport = async (groupId = null) => {
      try {
        setTablePayments([]);
        setIsLoading(true);
        ...
        if (groupId) {
          // ✅ Group-wise payments
          //const url = `/payment-out/get-group-payment-out/?group_id=${groupId}&pay_for=${paymentType}`;
          let url = `/payment-out/get-group-payment-out/?group_id=${groupId}`;
          if (paymentType) {
            url += `&pay_for=${paymentType}`;
          }
          ...
          if (response.data && response.data.length > 0) {
            const formattedData = response.data.map((group, index) => {
              ...
              return {
                ...
                old_receipt: group.old_receipt_no || "-",
                amount: group.amount || 0,
                date: group?.pay_date ? group.pay_date.split("T")[0] : "-",
                transaction_date: group?.createdAt
          ...
          // ✅ All payments
          const response = await api.get(
            "/payment-out/get-all-payment-out-report",
            {
              params: {
                pay_for: paymentType,
              },
  ...
  const handleGroupPaymentChange = async (groupId) => {
    ...
    if (groupId) {
      ...
      if (groupId === "today") {
        ...
        url = `/payment-out/get-group-payment-out/?group_id=${groupId}`;
        if (paymentType) {
          url += `&pay_for=${paymentType}`;
        }
        setEnableGroupColumn(false);
