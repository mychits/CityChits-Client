  ...
  async function getTransactions() {
    try {
      ...
      const filteredData = transactionsData.map((order, index) => {
        ...
        const color = status === "ACTIVE" ? "violet" : status === "PAID" ? "green" : "red";
            ...
            <div className="flex items-center gap-3">
              <Tooltip title={order.link_url ? "Copy Link" : "No Link"}>
                <button
                  ...
                  className={`p-2 rounded-full transition-colors ${order.link_url ? 'text-violet-600 hover:bg-violet-50' : 'text-gray-300'}`}
                >
